...
MSE = mse(y_true=y_test, y_pred=y_test_pred).item()
print('MSE:', MSE)
MSE_reg = mse(y_true=y_test, y_pred=y_test_pred_reg).item()
...
plt.plot(X_underlying, y_underlying_pred, c='g', linestyle='-.', label='$deg. = 8$')
plt.plot(X_underlying, y_underlying_pred_reg, c='r', linestyle='-.', label='$deg. = 8, \ell_2 reg$')
plt.ylim(-1.8, 1.8)
plt.annotate('lambda={}'.format(reg_lambda), xy=(0.487, 1.53))
plt.legend()
plt.savefig('带正则项的多项式回归.jpg')
plt.show()
